...
/**
 * Mind map for a single concept — Concept 2.0 architecture change §1/§15.
 * A USER-CREATED STUDY WORKSPACE, full stop. The auto-generated "Study
 * Map" (previously built from MeSH/Europe PMC data in studyMap.ts) has
 * been removed entirely — it just reproduced Learn-tab sentences as
 * nodes, which defeated the point of a mind map. This view now shows
 * only what the person actually built.
 *
 * Plain vertical/indented tree (flexbox, not SVG): wraps naturally on
 ...
 * no invented categories, and no scientific/literature edges of any
 * kind.
 *
...
export function ConceptMindMap({ root, concept }: ConceptMindMapProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <h3 className="font-ui text-micro font-medium uppercase tracking-wide text-ink-tertiary">My concept map</h3>

      {root.children.length === 0 ? (
        <EmptyState
          icon={<TreeStructure size={32} />}
          title="No concept map yet."
          description="Build your own map as you study this concept. Try starting with the concept in the center, then branch into structure, function, process, and applications — connect it to another concept from the Connections tab, or add a node of your own above."
        />
      ) : (
